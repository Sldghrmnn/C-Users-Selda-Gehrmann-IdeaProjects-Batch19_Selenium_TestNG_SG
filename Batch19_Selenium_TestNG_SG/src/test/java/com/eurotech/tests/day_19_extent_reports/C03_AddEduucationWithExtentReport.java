...
public class C03_AddEduucationWithExtentReport extends TestBase {
    ...
    public void addEducationTest() {
        ...
        addEducationPage.fillingAddEducationForm("KadirHas", "Lise", "Sayisal", "10091990", "01011993", "Hard Program");

        extentLogger.info("Verify that added education record can be seen at user profile page... (assert with school name)");
        String lastAddedSchoolName = userProfilePage.lastAddedSchoolName("KadirHas");
        String exceptedSchoolName = "KadirHas";
        Assert.assertEquals(lastAddedSchoolName, exceptedSchoolName);

        extentLogger.info("Delete last added education record");
        userProfilePage.deleteLastAddedInfoRecord("KadirHas");
